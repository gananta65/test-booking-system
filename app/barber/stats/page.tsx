  ...
  async function fetchStats() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const upcomingBookings = bookings.filter((b) => b.status !== "cancelled" && new Date(b.date) > now).length
      ...
      <header className="border-b border-border">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <nav className="flex gap-4">
            ...
            <Link href="/barber/bookings" className="text-muted-foreground hover:text-foreground">
              Bookings
            </Link>
          </nav>
        </div>
