...
export default function BarbersPage() {
  ...
  const filteredBarbers = barbers.filter((barber) => barber.user.name.toLowerCase().includes(search.toLowerCase()))
            ...
            {filteredBarbers.map((barber) => (
              <Card key={barber.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                ...
                <CardContent className="space-y-4">
                  {barber.description && <p className="text-sm text-muted-foreground">{barber.description}</p>}
